...
struct SMSHOSTLIB_STATISTICS_S {
	...
	valid only for DVB-T/H*/
	u32 GuardInterval;
	/*!< Guard Interval from SMSHOSTLIB_GUARD_INTERVALS_ET,
	valid only for DVB-T/H*/
	u32 CodeRate;
	/*!< Code Rate from SMSHOSTLIB_CODE_RATE_ET,
	valid only for DVB-T/H*/
	u32 LPCodeRate;
	/*!< Low Priority Code Rate from SMSHOSTLIB_CODE_RATE_ET,
	valid only for DVB-T/H*/
	u32 Hierarchy;
	...
	u32 BurstSize;
	/*!< Current burst size in bytes, valid only for DVB-H*/
	u32 BurstDuration;
	/*!< Current burst duration in mSec, valid only for DVB-H*/
	u32 BurstCycleTime;
	/*!< Current burst cycle time in mSec, valid only for DVB-H*/
	u32 CalculatedBurstCycleTime;
	/*!< Current burst cycle time in mSec,
	as calculated by demodulator, valid only for DVB-H*/
	u32 NumOfRows;
	/*!< Number of rows in MPE table, valid only for DVB-H*/
	u32 NumOfPaddCols;
	/*!< Number of padding columns in MPE table, valid only for DVB-H*/
	u32 NumOfPunctCols;
	/*!< Number of puncturing columns in MPE table, valid only for DVB-H*/
	u32 ErrorTSPackets;
	...
	/*!< Number of MPE tables which include errors after MPE RS decoding*/
	u32 NumOfCorrectedMpeTlbs;
	/*!< Number of MPE tables which were corrected by MPE RS decoding*/
	/* Common params*/
	...
	u32 DvbhSrvIndHP;
	/*!< DVB-H service indication info, bit 1 - Time Slicing indicator,
...
struct SMSHOSTLIB_ISDBT_LAYER_STAT_S {
	...
	/*!< Number of segments in layer A, 255 means layer does not exist*/
	u32 TMCCErrors;
	/*!< TMCC errors*/
};
...
struct SMSHOSTLIB_STATISTICS_ISDBT_S {
	u32 StatisticsType;
	/*!< Enumerator identifying the type of the structure.
	Values are the same as SMSHOSTLIB_DEVICE_MODES_E*/
	/*!< This field MUST always first in any statistics structure*/

	u32 FullSize;
	/*!< Total size of the structure returned by the modem.
	If the size requested by*/
	...
	/*!< Per-layer statistics, see struct SMSHOSTLIB_ISDBT_LAYER_STAT_S*/
