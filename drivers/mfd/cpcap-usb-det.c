...
#define TTU_TTA_CHRG_DET_N_GPIO  34
...
void enable_tta(void)
{
	...
	gpio_direction_input(TTU_TTA_CHRG_DET_N_GPIO);
}
EXPORT_SYMBOL(enable_tta);

void disable_tta(void)
{
	gpio_direction_output(TTU_TTA_CHRG_DET_N_GPIO, 1);
}
...
{
	return gpio_get_value(TTU_TTA_CHRG_DET_N_GPIO);
}
EXPORT_SYMBOL(value_of_gpio34);

void disable_tta_irq(void)
{
	disable_irq(gpio_to_irq(TTU_TTA_CHRG_DET_N_GPIO));
}
