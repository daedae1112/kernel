	...
	while (size && rc >= 0) {
		...
		if ((coredev->device_flags & SMS_ROM_NO_RESPONSE) &&
			...
			rc = coredev->sendrequest_handler(coredev->context,
				DataMsg, DataMsg->xMsgHeader. msgLength);
		}
		else
...
static inline char *sms_get_fw_name(struct smscore_device_t *coredev, int mode,
	...
	if (board_id == SMS_BOARD_UNKNOWN) {
		sms_debug("getting fw name from lookup \
			table mode %d type %d", mode, type);
		return smscore_fw_lkup[mode][type];
	}

	sms_debug("getting for fw name from sms_boards \
		board_id %d mode %d", board_id, mode);
	fw = sms_get_board(board_id)->fw;
	if (fw == NULL) {
		sms_debug("cannot find fw name in sms_boards, \
		getting from lookup table mode %d type %d", mode, type);
		return smscore_fw_lkup[mode][type];
	}
