				     ...
				     enum led_brightness value)
{
	...
	if (lm3530_debug)
		pr_info("%s: value = %d cur mode = %d\n",
			__func__, value, als_data->mode);
...
/* Skip ALS reporting in this case =======================
				   ...
				   *attr, const char *buf, size_t size)
{
	...
	if (lm3530_debug)
		pr_info("%s: req mode = %d\n", __func__, (uint32_t)mode_value);
				   ...
				   *attr, const char *buf, size_t size)
{
	...
	if (lm3530_debug)
		pr_info("%s: req pwm = %d cur mode = %d\n",
			__func__, (uint32_t)pwm_value, als_data->mode);
...
void ld_lm3530_work_queue(struct work_struct *work)
{
	...
	if (lm3530_debug)
		pr_info("%s:ALS Zone read back: 0x%x\n",
		       __func__, als_data->zone);

	if (als_data->mode == AUTOMATIC) {
		if (als_data->zone & LM3530_ALS_ZONE_CHANGE_MASK) {
			ret = lm3530_write_reg(als_data, LM3530_ALS_CONFIG,
				LM3530_AUTO_SLOW_VALUE);
			if (lm3530_debug)
				pr_info("%s:ALS config slow\n", __func__);
		} else {
			ret = lm3530_write_reg(als_data, LM3530_ALS_CONFIG,
				LM3530_AUTO_FAST_VALUE);
			if (lm3530_debug)
				pr_info("%s:ALS config fast\n", __func__);
		}
		if (ret != 0) {
			pr_err("%s:Unable to write ALS config: %d\n",
				__func__, ret);
			enable_irq(als_data->client->irq);
			return;
		}
