...
const u16 lp3907_pwr_off_tbl_reg[] = {
	LP3907_BKLDOEN_REG,
	LP3907_BKLDOEN_REG,
	LP3907_BKLDOEN_REG,
...
const u8 lp3907_pwr_off_tbl_data[] = {
	0x70,    /* BUCK1 disable */
	0x60,    /* LDO1EN disable */
	0x20,    /* LDO2EN disable */
...
static int lp3907_power_on_sequence(struct lp3907_data *pLP3907)
{
	...
	for (; lp3907_init_tbl_reg[i] != LP3907_INVALID_REG; i++) {
		if (ret == 0) {
			ret = lp3907_i2c_write(pLP3907, \
					       lp3907_init_tbl_reg[i], \
					       lp3907_init_tbl_data[i]);
			if (!ret) {
...
lp3907_configure_exit:
	...
	dev_err(&pLP3907->client->dev, \
		"LP3907-CONFIGURE: failed [%d] line=[%d]\n", ret, line);
	return ret;
}
...
static int lp3907_power_off_sequence(struct lp3907_data *pLP3907)
{
	...
	for (; lp3907_pwr_off_tbl_reg[i] != LP3907_INVALID_REG; i++) {
		if (ret == 0) {
			ret = lp3907_i2c_write(pLP3907, \
					lp3907_pwr_off_tbl_reg[i], \
					lp3907_pwr_off_tbl_data[i]);
			if (!ret)
				dev_dbg(&pLP3907->client->dev, \
				"[LP3907]WR: reg = 0x%x, data = 0x%x\n", \
				lp3907_pwr_off_tbl_reg[i], \
			...
			dev_err(&pLP3907->client->dev, \
				"lp3907_i2c_write(%x, %x) is failed!\n", \
				lp3907_pwr_off_tbl_reg[i], \
				lp3907_pwr_off_tbl_data[i]);
			line = __LINE__;
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	if (pLP3907 == NULL) {
		printk(KERN_ERR "[LP3907] *pLP3907 is null\n");
		return status;
	}
...
const struct file_operations lp3907_fops = {
	.owner   = THIS_MODULE,
	.open    = lp3907_open,
			...
			const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data == NULL) {
		dev_err(&client->dev, \
			"[lp3907]platform data is NULL. exiting.\n");
		ret = -ENODEV;
		goto err0;
	}
	...
	if (pLP3907->pdata == NULL)
		goto err1;
	memcpy(pLP3907->pdata, client->dev.platform_data, \
		sizeof(*pLP3907->pdata));
	...
	if (ret < 0) {
		dev_err(&pLP3907->client->dev,
			"LP3907 chip configure is failed!\n");
		goto err2;
	}
