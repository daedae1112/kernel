...
#define CLK_SEL        "clk_sel"
#define PCM_I2S_SEL    "pcm_i2s_sel"
...
struct cpcap_audio_state cpcap_audio_state = {
	...
	CPCAP_AUDIO_HDTV_OFF, /*w21772_hdmi*/
        CPCAP_AUDIO_MIC_MUTE
};
...
/*w21772_hdmi*/
static int store_stdacmode_hdtv = CPCAP_AUDIO_STDAC_OFF;
...
/*w21772_hdmi*/
#if 1
static int hdtv_configure_ssi(struct cpcap_audio_state *state)
{
	unsigned int ssi = STDAC_SSI;
	if (cpcap_audio_state.hdtv_mode == CPCAP_AUDIO_HDTV_ON) { /*I2S Mode*/
		tx_cfg_params.word_length1 = OMAP_MCBSP_WORD_16;
		tx_cfg_params.word_length2 = OMAP_MCBSP_WORD_16;
		tx_cfg_params.frame_length1 = OMAP_MCBSP_FRAMELEN_N(1);
		tx_cfg_params.frame_length2 = OMAP_MCBSP_FRAMELEN_N(1);
		tx_params.word_length1 = OMAP_MCBSP_WORD_16;
		/* 1 bit delay */
		tx_cfg_params.data_delay   = OMAP_MCBSP_DATADELAY1;
		tx_cfg_params.fs_polarity  = OMAP_MCBSP_FS_ACTIVE_LOW;
		tx_cfg_params.phase = OMAP_MCBSP_FRAME_DUALPHASE;
	} else {
		tx_cfg_params.word_length1 = OMAP_MCBSP_WORD_32;
		tx_cfg_params.word_length2 = OMAP_MCBSP_WORD_8;
		tx_cfg_params.data_delay   = OMAP_MCBSP_DATADELAY1;
		tx_cfg_params.phase        = OMAP_MCBSP_FRAME_SINGLEPHASE;
		tx_params.word_length1 = OMAP_MCBSP_WORD_32;
		tx_cfg_params.fs_polarity  = OMAP_MCBSP_FS_ACTIVE_HIGH;
	}
...
static int hdtv_stop_ssi(struct cpcap_audio_state *state)
{
	int ssi = STDAC_SSI;
...
/*w21772_hdmi*/
		store_stdacmode_hdtv = CPCAP_AUDIO_STDAC_ON;
		map_audioic_speakers();
		cpcap_audio_set_audio_state(&cpcap_audio_state);
	}
out:
...
/*w21772_hdmi*/
		store_stdacmode_hdtv = CPCAP_AUDIO_STDAC_OFF;
...
/*		cpcap_audio_state.input_gain_l = (gain & 0xFF00) >> 8;*/
		cpcap_audio_state.input_gain_l = gain & 0xFF;
		cpcap_audio_set_audio_state(&cpcap_audio_state);
...
/*w21772_hdmi*/
#if 1
	case SOUND_MIXER_HDTV_ON:
	{

		int ret;
		if (state.stdac_out_stream != NULL) {
			AUDIO_LEVEL1_LOG("SOUND_MIXER_HDTV_ON In\n");
			cpcap_audio_state.hdtv_mode = CPCAP_AUDIO_HDTV_ON;
			store_stdacmode_hdtv = cpcap_audio_state.stdac_mode;
		cpcap_audio_state.stdac_mode = CPCAP_AUDIO_STDAC_CLOCK_ONLY;
			mutex_unlock(&audio_lock);
			ret = down_timeout(&sem_hdmi, AUDIO_TIMEOUT);
			mutex_lock(&audio_lock);
			if (!ret) {
				AUDIO_LEVEL1_LOG("discard buf and stop ssi\n");
				hdtv_stop_ssi(&cpcap_audio_state);
		audio_discard_buf(state.stdac_out_stream, inode);
				state.stdac_out_stream->active = 0;
				up(&sem_hdmi);
			} else
				AUDIO_ERROR_LOG("HDTV Off: timedout\n");
			hdtv_configure_ssi(&cpcap_audio_state);
			cpcap_audio_set_audio_state(&cpcap_audio_state);
			AUDIO_LEVEL1_LOG("SOUND_MIXER_HDTV_ON Out\n");
		}

		break;
	}

	case SOUND_MIXER_HDTV_OFF:
	{

		int ret;
		if (state.stdac_out_stream != NULL) {
			AUDIO_LEVEL1_LOG("SOUND_MIXER_HDTV_OFF In\n");
			cpcap_audio_state.hdtv_mode = CPCAP_AUDIO_HDTV_OFF;
		cpcap_audio_state.stdac_mode = store_stdacmode_hdtv;
			mutex_unlock(&audio_lock);
			ret = down_timeout(&sem_hdmi, AUDIO_TIMEOUT);
			mutex_lock(&audio_lock);
			if (!ret) {
		...
		audio_discard_buf(state.stdac_out_stream, inode);
			...
			hdtv_configure_ssi(&cpcap_audio_state);
			cpcap_audio_set_audio_state(&cpcap_audio_state);
			AUDIO_LEVEL1_LOG("SOUND_MIXER_HDTV_OFF Out\n");
		}
        ...
        case SOUND_MIXER_MIC_MUTE:
	{
		...
		TRY(copy_from_user(&mic_mute, (unsigned int *)arg,
						sizeof(unsigned int)))
		AUDIO_LEVEL1_LOG("SOUND_MIXER_MIC_MUTE, mic mute = %d\n", mic_mute);
		cpcap_audio_state.mic_mute = mic_mute;		
		cpcap_audio_set_audio_state(&cpcap_audio_state);		
		break;
	}
...
/*w21772_hdmi*/
	...
	if (ret) {
		AUDIO_ERROR_LOG("audio_write: sem_hdmi -- timedout\n");
		ret = count;
		goto out1;
	}
...
static int audio_codec_open(struct inode *inode, struct file *file)
{
	...
	if ((file->f_flags & O_TRUNC) != 0)
        ...
        cpcap_audio_state.mic_mute = CPCAP_AUDIO_MIC_UNMUTE;
