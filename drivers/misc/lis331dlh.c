...
static int lis331dlh_get_acceleration_data(struct lis331dlh_data *lis, int *xyz)
{
	...
	if (old_brdver) {
		xyz[0] = ((lis->pdata->negate_x)
			? (-hw_d[lis->pdata->axis_map_x])
			: (hw_d[lis->pdata->axis_map_x]));
		xyz[1] = ((lis->pdata->negate_y)
			? (-hw_d[lis->pdata->axis_map_y])
			: (hw_d[lis->pdata->axis_map_y]));
		xyz[1] *= -1;
		xyz[2] *= -1;
	} else {
		xyz[1] = ((lis->pdata->negate_x)
			? (-hw_d[lis->pdata->axis_map_x])
			: (hw_d[lis->pdata->axis_map_x]));
		xyz[0] = ((lis->pdata->negate_y)
			? (-hw_d[lis->pdata->axis_map_y])
			: (hw_d[lis->pdata->axis_map_y]));
		xyz[0] *= -1;
	}
...
static int __init lis331dlh_init(void)
{
	...
	old_brdver = 0;
	if (strstr(boot_command_line, "P1_TTNM"))
		old_brdver = 1;
